...
from pyAyiin.lib.tools import install_requirements
from pyAyiin.database.variable import delVar, setVar
...
async def deploy(xx, repo, ups_rem, ac_br, txt):
    if ayiin._host == 'heroku':
        if ayiin.HEROKU_API_KEY is not None:
            import heroku3

            heroku = heroku3.from_key(ayiin.HEROKU_API_KEY)
            heroku_app = None
            heroku_applications = heroku.apps()
            if ayiin.HEROKU_APP_NAME is None:
                await eor(
                    xx,
            ...
            for app in heroku_applications:
                if app.name == ayiin.HEROKU_APP_NAME:
                    heroku_app = app
                    break
            if heroku_app is None:
                ...
                return repo.__del__()
            delVar("restartstatus")
            setVar("restartstatus", f"{xx.chat_id}\n{xx.id}")
            ups_rem.fetch(ac_br)
            repo.git.reset("--hard", "FETCH_HEAD")
            heroku_git_url = heroku_app.git_url.replace(
                "https://", "https://api:" + ayiin.HEROKU_API_KEY + "@"
            )
...
@ayiinCmd(pattern="update( now| deploy|$)")
    ...
    if conf == "now":
        for commit in changelog.splitlines():
            if (
                ...
                and ayiin.HEROKU_APP_NAME is not None
                and ayiin.HEROKU_API_KEY is not None
            ):
                return await xx.edit(
