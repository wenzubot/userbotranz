...
from pyAyiin.lib.tools import load_module, remove_plugin, reply_id

from . import cmd


@ayiinCmd(pattern="install$")
async def _(event):
    if event.fwd_from:
        return
...
@ayiinCmd(pattern="uninstall (?P<shortname>\\w+)")
