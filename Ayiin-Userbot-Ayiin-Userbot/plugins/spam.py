...
from pyAyiin.database.variable import setVar, getVar
...
async def spam_function(event, spammer, xnxx, sleeptimem, sleeptimet, DelaySpam=False):
    ...
    if len(xnxx) == 2:
        ...
        if ayiin.BOTLOG_CHATID:
            ...
            spammer = await event.client.send_file(ayiin.BOTLOG_CHATID, spammer)
            await unsavegif(event, spammer)
        return
...
async def stickerpack_spam(event):
    ...
    if ayiin.BOTLOG_CHATID:
        ...
        await event.client.send_file(ayiin.BOTLOG_CHATID, reqd_sticker_set.documents[0])


@ayiinCmd(pattern="cspam ([\\s\\S]*)")
...
@ayiinCmd(pattern="(delayspam|dspam) ([\\s\\S]*)")
