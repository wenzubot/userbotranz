...
@ayiinCmd(pattern=r"manga ?(.*)")
async def search_manga(message):
    search_query = message.pattern_match.group(1)
    await message.get_reply_message()
    xx = await eor(message, "`Searching Manga..`")
    jikan = jikanpy.jikan.Jikan()
...
@ayiinCmd(pattern="a(kaizoku|kayo) ?(.*)")
async def site_search(event):
    ...
    if search_query:
        ...
        return await eod(event, "`Uuf Bro.. Gib something to Search`")
    if site == "kaizoku":
        ...
        for entry in search_result:
            ...
            post_name = html.escape(entry.text.strip())
            result += f"• <a href='{post_link}'>{post_name}</a>\n"
            await eor(event, result, parse_mode="HTML")


@ayiinCmd(pattern="char ?(.*)")
async def character(event):
    ...
    if search_query:
        ...
        await eor(event, "Format: `.char <character name>`")
        return
    xx = await eod(event, "`Searching Character...`")
    try:
...
async def upcoming(message):
    ...
    for new in anime:
        ...
        if len(rep) > 1000:
            break
        await eor(message, rep, parse_mode="html")


@ayiinCmd(pattern="whatanime$")
async def whatanime(e):
    ...
    if not media:
        await eod(e, "`Media required`")
        return
    ig = is_gif(media) or is_video(media)
    if not is_image(media) and not ig:
        await eod(e, "`Media must be an image or gif or video`")
        return
    filename = "file.jpg"
    if not ig and isinstance(media, MessageMediaDocument):
        ...
        for i in attribs:
            if isinstance(i, DocumentAttributeFilename):
                ...
                break
    xx = await eor(e, "`Downloading image..`")
