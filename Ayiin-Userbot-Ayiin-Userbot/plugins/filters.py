...
from pyAyiin.database.filters import addFilter, delFilter, getAllFilters
...
def check_for_filters(chat_id, trigger):
    all_filters = getAllFilters(chat_id)
...
@ayiinHandler(incoming=True, disable_edited=True)
async def filter_incoming_handler(handler):
    """Checks if the incoming message contains handler of a filter"""
    try:
        if not (await handler.get_sender()).bot:
            name = handler.raw_text
            filters = getAllFilters(handler.chat_id)
            if not filters:
                return
            for trigger in filters:
                ...
                if pro and trigger[3]:
                    msg_o = await handler.client.get_messages(
                        entity=ayiin.BOTLOG_CHATID, ids=int(trigger[3])
                    )
...
async def add_new_filter(new_handler):
    ...
    if new_handler.chat_id in ayiin.BLACKLIST_CHAT:
    ...
    addFilter(new_handler.chat_id, keyword, string, msg_id)
    if not cek:
        caption = f"**Berhasil Menambahkan Filter** `{keyword}` **Disini**"
    else:
        caption = f"**Berhasil Memperbarui Filter** `{keyword}` **Disini**"
    await new_handler.edit(caption)
...
async def remove_a_filter(r_handler):
    ...
    if not cek:
        ...
        delFilter(r_handler.chat_id, filt)
        await r_handler.edit(
            f"**Berhasil Menghapus Filter** `{filt}`"
        )


@ayiinCmd("delfilterbot (.*)")
...
async def filters_active(event):
    ...
    filters = getAllFilters(event.chat_id)
    if filters:
        for filt in filters:
            transact += f" ⍟ `{filt[1]}`\n"
