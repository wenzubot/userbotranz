...
async def time_func(tdata):
    ...
    if len(con) > 4:
        ...
        c_name = ayiin.COUNTRY
        tz_num = ayiin.TZ_NUMBER
        timezones = await get_tz(ayiin.COUNTRY)
    else:
    ...
    if ayiin.COUNTRY:
        await tdata.edit(
            f"**Sekarang Tanggal** `{dtnow}` **di {ayiin.COUNTRY} ({time_zone} timezone.)**"
        )
        return


@ayiinCmd(pattern="date(?: |$)(.*)(?<![0-9])(?: |$)([0-9]+)?")
