...
from pyAyiin.lib.tools import humanbytes, progress, run_cmd
from pyAyiin.lib.FastTelethon import download_file, upload_file

from . import cmd


@ayiinCmd(pattern="download(?: |$)(.*)")
        ...
        if "|" in input_str:
            ...
            if head and not os.path.isdir(
                os.path.join(
                    ...
                    head)):
                os.makedirs(os.path.join(ayiin.TEMP_DOWNLOAD_DIRECTORY, head))
                file_name = os.path.join(head, tail)
        downloaded_file_name = ayiin.TEMP_DOWNLOAD_DIRECTORY + "" + file_name
        ...
        try:
            ...
            if hasattr(media, "document"):
                    ...
                    if "audio" in mime_type:
                        ...
                        filename = (
                            ...
                            datetime.now().isoformat(
                                ...
                                "seconds") +
                            ".mp4")
                outdir = ayiin.TEMP_DOWNLOAD_DIRECTORY + filename
                c_time = time.time()
                ...
                result = await target_file.client.download_media(
                    media, ayiin.TEMP_DOWNLOAD_DIRECTORY
                )
...
async def get_video_thumb(file, output):
    ...
    if os.path.lexists(output):
        return output
    ayiin.log.info(t_resp)
    ayiin.log.info(e_resp)
    return None


@ayiinCmd(pattern="upload (.*)")
