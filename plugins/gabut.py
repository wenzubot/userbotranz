...
@ayiinCmd(pattern="teemo(?: |$)(.*)")
async def _(teemo):
    yins = await eor(teemo, "`𝙏𝙚𝙚𝙢𝙢𝙤𝙤 𝙈𝙪𝙡𝙪 𝙇𝙪 😏`")
    sleep(2)
...
@ayiinCmd(pattern="give(?: |$)(.*)")
async def _(giveaway):
    ayiin = await eor(giveaway, "`𝙎𝙮𝙖𝙧𝙖𝙩 𝙄𝙠𝙪𝙩 𝙂𝙞𝙥𝙚𝙚𝙬𝙚𝙮`")
    sleep(2)
...
@ayiinCmd(pattern="uno(?: |$)(.*)")
async def _(uno):
    xd = await eor(uno, "`𝙆𝙖𝙠𝙠𝙠 👉👈`")
    sleep(2)
