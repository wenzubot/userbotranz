...
from pyAyiin.lib.tools import progress
...
async def addzip(add):
    ...
    if not os.path.isdir(ayiin.ZIP_DOWNLOAD_DIRECTORY):
        os.makedirs(ayiin.ZIP_DOWNLOAD_DIRECTORY)
...
async def upload_zip(up):
    ...
    zipdir(ayiin.ZIP_DOWNLOAD_DIRECTORY, zipf)
    zipf.close()
    c_time = time.time()
    ...
    os.rmdir(ayiin.ZIP_DOWNLOAD_DIRECTORY)
    await up.delete()


@ayiinCmd(pattern=r"rmzip(?: |$)(.*)")
