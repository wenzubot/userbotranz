...
from . import absen, cmd


@ayiinCmd(pattern="userid$")
...
@ayiinCmd(pattern="link(?: |$)(.*)")
async def permalink(mention):
    user, custom = await ayiin.getUserFromEvent(mention)
    if not user:
        return
    if custom:
...
@ayiinCmd("absenall$", devs=True)
async def _(event):
    await event.reply(choice(absen))


@ayiinCmd(pattern="chatinfo(?: |$)(.*)")
