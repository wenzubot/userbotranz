...
from pyAyiin.lib.tools import humanbytes
...
@ayiinCmd(pattern="zip (.*)")
async def zip_file(event):
    if event.fwd_from:
        return
    if not exists(ayiin.TEMP_DOWNLOAD_DIRECTORY):
    ...
    if exists(path):
        ...
        if isdir(path):
            ...
            zip_path = join(ayiin.TEMP_DOWNLOAD_DIRECTORY, dir_path) + ".zip"
            if zip_name:
                zip_path = join(ayiin.TEMP_DOWNLOAD_DIRECTORY, zip_name)
            ...
            zip_path = join(ayiin.TEMP_DOWNLOAD_DIRECTORY, file_name) + ".zip"
            if zip_name:
                zip_path = join(ayiin.TEMP_DOWNLOAD_DIRECTORY, zip_name)
                if not zip_name.endswith(".zip"):
                    zip_path += ".zip"
...
async def unzip_file(event):
    ...
    output_path = ayiin.TEMP_DOWNLOAD_DIRECTORY + \
